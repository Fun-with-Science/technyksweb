  ...
  template: `
    <div class="px-6 md:px-16 pt-24 pb-20 max-w-7xl mx-auto">
      <div class="text-center max-w-3xl mx-auto mb-16">
        <div class="inline-flex items-center gap-2 font-['JetBrains_Mono'] text-xs text-[#3B82F6] px-3.5 py-1.5 border border-[#3B82F6]/30 bg-[#3B82F6]/10 rounded-full w-fit mb-4">
        ...
        <div class="inline-flex w-full max-w-md flex-col sm:flex-row items-stretch sm:items-center bg-[#121A2B] technical-border p-1.5 rounded-2xl sm:rounded-full mt-8">
          <button type="button" (click)="isAnnual.set(false)" [class.bg-[#3B82F6]]="!isAnnual()" [class.text-[#040810]]="!isAnnual()" [class.font-bold]="!isAnnual()" [class.text-[#d9c3af]]="isAnnual()" class="font-['JetBrains_Mono'] text-xs uppercase px-4 sm:px-6 py-2.5 rounded-full transition-all whitespace-nowrap">Monthly Billing</button>
          <button type="button" (click)="isAnnual.set(true)" [class.bg-[#3B82F6]]="isAnnual()" [class.text-[#040810]]="isAnnual()" [class.font-bold]="isAnnual()" [class.text-[#d9c3af]]="!isAnnual()" class="font-['JetBrains_Mono'] text-xs uppercase px-4 sm:px-6 py-2.5 rounded-full transition-all flex items-center justify-center gap-2 whitespace-nowrap">Annual Billing <span class="membership-save-badge text-[10px] px-2 py-0.5 rounded-full">SAVE 33%</span></button>
        </div>
      </div>

      <div class="mb-12 max-w-4xl mx-auto p-4 bg-[#121A2B] border border-[#3B82F6]/40 rounded text-xs font-['JetBrains_Mono'] text-[#a1c9ff] flex items-start sm:items-center gap-3">
        <span class="material-symbols-outlined text-xl text-[#3B82F6]">security</span>
        <div><span class="font-bold uppercase text-[#3B82F6]">Secure billing:</span> Membership access is granted after confirmed payment and is enforced by the server for every protected lesson.</div>
      </div>
            ...
            <article class="bg-[#121A2B] technical-border rounded p-8 flex flex-col justify-between">
              <div>
                ...
                <ul class="flex flex-col gap-3 font-['Inter'] text-sm text-[#d9c3af] mb-8">
                  @for (feature of plan.features; track feature) { <li class="flex items-center gap-2"><span class="material-symbols-outlined text-[#3B82F6] text-base">check</span>{{ feature }}</li> }
                </ul>
              </div>
              <a routerLink="/auth/signup" class="w-full text-center font-['JetBrains_Mono'] text-xs uppercase tracking-wider text-[#3B82F6] border border-[#3B82F6] py-3.5 rounded font-semibold hover:bg-[#3B82F6]/10 transition-colors">Get Started Free</a>
            </article>
          }

          @if (paidPlan(); as plan) {
            <article class="lg:col-span-2 bg-[#121A2B] border-2 border-[#3B82F6] rounded p-8 flex flex-col justify-between relative shadow-2xl">
              <span class="absolute -top-3.5 right-6 font-['JetBrains_Mono'] text-[10px] uppercase font-bold text-[#040810] bg-[#3B82F6] px-3 py-1 rounded-full shadow">{{ isAnnual() ? 'BEST VALUE' : 'POPULAR' }}</span>
              <div>
                <span class="font-['JetBrains_Mono'] text-xs uppercase text-[#3B82F6] font-bold">{{ isAnnual() ? 'ANNUAL VIP' : 'PRO ENGINEER' }}</span>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 font-['Inter'] text-sm text-[#d9c3af] mb-8">
                  @for (feature of plan.features; track feature) { <div class="flex items-center gap-2"><span class="material-symbols-outlined text-[#3B82F6] text-base">check_circle</span>{{ feature }}</div> }
                </div>
              </div>
              <a routerLink="/checkout" [queryParams]="{ planSlug: plan.slug }" class="w-full text-center font-['JetBrains_Mono'] text-xs uppercase tracking-wider text-[#040810] bg-[#3B82F6] py-4 rounded font-bold hover:bg-[#2563eb] transition-all flex items-center justify-center gap-2 shadow-lg">Join {{ plan.name }} <span class="material-symbols-outlined text-sm">arrow_forward</span></a>
            </article>
          } @else {
