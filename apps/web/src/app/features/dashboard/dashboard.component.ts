...
@Component({
  ...
  template: `
    <div class="px-6 md:px-16 pt-24 pb-20 max-w-7xl mx-auto">
      <!-- Welcome Header -->
      <div class="mb-12 border-b border-[#1E293B] pb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p class="font-['Inter'] text-sm text-[#d9c3af] mt-1">
            Track your progress across enrolled software architecture tracks.
          </p>
        </div>
          ...
          @for (enrollment of enrollments(); track enrollment.id) {
            <div class="bg-[#121A2B] technical-border rounded p-6 flex flex-col justify-between shadow-xl">
              <div>
                ...
                <p class="font-['Inter'] text-xs text-[#d9c3af] mb-6 line-clamp-2">
                  {{ enrollment.course.subtitle }}
                </p>

                <!-- Segment Progress Bar (Design Spec) -->
                <div class="mb-6">
                  <div class="flex justify-between text-[11px] font-['JetBrains_Mono'] text-[#a18d7b] mb-2">
                    <span>PROGRESS TRACKER</span>
                    <span>{{ enrollment.completedLessonIds.length }} Lessons Finished</span>
                  </div>
                  <div class="h-3 w-full bg-[#040810] border border-[#1E293B] rounded p-0.5 flex gap-1">
                    <div
